    ...
    def get_application_scores(qry):
        ...
        for ln in qry:
            if ApplicationScores.objects.filter(loan_id=ln.id).exists():
                ...
                prediction['officer_id'] = 1
                prediction['cust_id'] = 1
                prediction['company_id'] = 1
                
                ap_score = ApplicationScores(
                                    loan_id  =  ln.LOAN_ID,
                                    client_id = prediction['cust_id'],
                                    officer_id = prediction['officer_id'],
        ...
        for ln in qry:
            if RetentionScores.objects.filter(loan_id=ln.id).exists():
                ...
                retent_scrores = RetentionScores(
                                ...
                                retention_loan_num  = prediction['retention_loan_num'],
